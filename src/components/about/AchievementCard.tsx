...
export default function AchievementCard({ data, isFlipped: externalFlipped, isActive, onFlip }: AchievementCardProps) {
  ...
  useEffect(() => {
    ...
    startSpin();
    interval = setInterval(startSpin, 3000); // a cada 3s
    return () => {
  ...
  return (
    ...
    >
      <div
        className={`achievement-card relative w-full aspect-[4/5] transition-transform duration-700 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''
          }`}
      >
        ...
        <div className="achievement-card-face achievement-card-front absolute w-full h-full backface-hidden rounded-lg overflow-hidden shadow-lg bg-[var(--bg-secondary)]">
          ...
          {/* Indicador de rotação (agora dentro do bloco da mensagem) */}
          {!isActive && (
